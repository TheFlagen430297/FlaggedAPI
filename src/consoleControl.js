 ...
 * @param {String} dir Use `__dirname` only
...
function consoleControl(dir, imports) {
    ...
    function run() {
        let xEnvs = {
            console: {
                ...
                blinkingInterval: setInterval(() => {
                    ...
                    if (xEnvs.console.paused) {
                        log()
                        xEnvs.console.paused = false;
                    }
        ...
        process.stdin.on('keypress', function (ch, key) {
            ...
            if (!key) {
                ...
                if (key.name == `return`) {
                    ...
                    editString({ type: `bs`, info: null });
                } else {
                    editString({ type: `add`, info: key.sequence });
                }
            }
            ...
            function editString(input) {
                if (input.type == `bs`) {
                    ...
                    xEnvs.console.currentString = xEnvs.console.currentString + input.info;
                    runLog();
                }
            }
        ...
        console.log = function() {
            ...
            if (arguments[2] == logID) return

            // Do something with the data (args) here
            // For example, you could log to a file, send to a server, etc.
            // In this example, let's just log to the console with an additional prefix

            logID++;
            xEnvs.console.prompted = false
            xEnvs.console.paused = true;
            originalLog.apply(console, args, logID); 
            xEnvs.console.prompted = true
        };
    }
}
module.exports = { consoleControl, enabled };